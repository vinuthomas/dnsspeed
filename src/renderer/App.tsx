...
function getSparklineColor(val: number): string {
...
function getMedianTextColor(median: number): string {
  if (median === -1) return 'text-red-400';
  if (median < 20) return 'text-emerald-400';
  if (median < 50) return 'text-yellow-400';
        ...
        if (val === -1) {
          return (
            <div
              ...
              key={i}
              className="flex-1 bg-rose-500 h-full opacity-90 rounded-sm"
              title={`Test ${i + 1}: Timeout`}
            />
          );
        }
        const heightPct = Math.min((val / maxVal) * 100, 100);
        const color = getSparklineColor(val);
        return (
          <div
            // eslint-disable-next-line react/no-array-index-key
            key={i}
  ...
  const fetchCurrentDns = async () => {
    ...
    if (dns && dns.length > 0) {
      setProviders((prev) => {
        ...
        return [{ name: 'System DNS', servers: dns, isCustom: true }, ...prev];
      });
    }
  };
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <p className="text-slate-400 font-medium text-lg tracking-wide">
              Professional DNS Benchmarking Suite
            </p>
          </div>
          ...
          <div className="flex-1 w-full max-w-md ml-auto flex flex-col items-end relative z-10">
            ...
            >
              {testing
                ? `Running Test (${iterationsDone}/${TOTAL_ITERATIONS})`
                : 'Start Benchmark'}
            </button>
            {testing && (
            ...
            <div className="bg-slate-900 border border-emerald-500/20 rounded-2xl p-6 shadow-xl relative overflow-hidden">
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                {topServers.map((s, idx) => (
                  <div
                    key={s.ip}
                    className="bg-slate-950 border border-slate-800 p-5 rounded-xl flex flex-col items-center justify-center text-center relative group hover:border-emerald-500/50 transition-colors shadow-inner"
                  >
                    <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-emerald-500 to-teal-400 opacity-70" />
                    <span className="text-emerald-500/20 font-black text-5xl absolute top-2 right-2 select-none">
                      #{idx + 1}
                    </span>
                    <span className="text-emerald-400 font-bold text-3xl mb-1 mt-2 tracking-tight z-10">
                      {s.median}{' '}
                      <span className="text-base text-emerald-500/70 font-medium">
                        ms
                      </span>
                    </span>
                    <span className="font-semibold text-slate-200 z-10">
                      {s.providerName}
                    </span>
                    <span className="font-mono text-xs text-slate-500 mt-1.5 bg-slate-900 px-2 py-1 rounded-md z-10 border border-slate-800">
                      {s.ip}
                    </span>
                  </div>
                ))}
                {topServers.length === 0 && (
                  <p className="text-slate-400 col-span-5 py-4 text-center">
              ...
              {failedServers.length > 0 && (
                <div className="mt-8 pt-6 border-t border-slate-800/50">
                  <h3 className="text-sm font-bold text-red-400/80 mb-3 uppercase tracking-wider">
                    Unreachable Servers
                  </h3>
                  <div className="flex flex-wrap gap-2">
                    {failedServers.map((s) => (
                      <div
                        key={s.ip}
                        className="bg-red-950/30 border border-red-900/50 px-3 py-1.5 rounded-md flex items-center text-sm"
                      >
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-right font-bold text-base">
                            ...
                            {hasData && (
                              <span
                                className={getMedianTextColor(stats.median)}
                              >
                                {stats.median === -1
                                  ? 'N/A'
                                  : `${stats.median} ms`}
                ...
                <div className="space-y-1.5">
                  ...
                  >
                    ...
                    <input
                      ...
                      className="w-full bg-slate-950 border border-slate-700 rounded-xl px-4 py-2.5 text-slate-200 placeholder-slate-600 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-shadow mt-1.5"
                ...
                <div className="space-y-1.5">
                  ...
                  >
                    ...
                    <input
                      ...
                      className="w-full bg-slate-950 border border-slate-700 rounded-xl px-4 py-2.5 text-slate-200 font-mono text-sm placeholder-slate-600 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-shadow mt-1.5"
              ...
              {providers.some((p) => p.isCustom && p.name !== 'System DNS') && (
                <div className="mt-8 pt-6 border-t border-slate-800">
                  ...
                  <ul className="space-y-2">
                    {providers
                      .filter((p) => p.isCustom && p.name !== 'System DNS')
                      .map((p) => (
                        <li
                          key={p.name}
                          className="flex justify-between items-center text-sm bg-slate-950 border border-slate-800 rounded-lg p-2 px-3"
                        >
